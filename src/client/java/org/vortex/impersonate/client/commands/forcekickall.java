...
public class forcekickall {
    private static boolean forceKickAllAlreadyRegistered = false;
    private static Session oldSession;
    private static final AtomicBoolean enabledForceKickAll = new AtomicBoolean(false);
    public static void register(CommandDispatcher<FabricClientCommandSource> dispatcher, Event<ClientPlayConnectionEvents.Join> onJoin) {
        dispatcher.register(ClientCommandManager.literal("forcekickall")
                .then(ClientCommandManager.literal("opInjectMethod")
                        .then(ClientCommandManager.argument("playerWithOp", StringArgumentType.string())
                                .executes(forcekickall::execute))));
        if (!forceKickAllAlreadyRegistered) {
            forceKickAllAlreadyRegistered = true;
            onJoin.register((handler, sender, client1) -> {
                if (enabledForceKickAll.compareAndSet(true, false)) {
                    (new Thread(()->{
                        handler.sendChatCommand("kick @a");
                        try {
                            Thread.sleep(550);
                        } catch (InterruptedException e) {
                            throw new RuntimeException(e);
                        }
                        handler.getConnection().disconnect(Text.literal("Kicked."));
                        MinecraftAccesor clientAccessor = (MinecraftAccesor) client1;
                        clientAccessor.imp$session(oldSession);
                        enabledForceKickAll.set(false);
                    })).start();
                }
            });
        }
